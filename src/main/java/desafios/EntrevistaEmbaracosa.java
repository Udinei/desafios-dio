 ...
 * ela precisa da sua ajuda para escrever um programa que omita a parte repetida, de modo que as palavras sejam
 * pronunciadas como deveriam ser.
 *
 * Escreva um programa que, dada uma palavra errada, a mesma seja corrigida.
 *
 * Entrada
 * Haverá diversos casos de teste. Cada caso de teste é formado por uma palavra, de, no máximo, 30 caracteres, dita da
 * forma errada. A entrada termina com fim de arquivo.
 *
 * Saída
...
public class EntrevistaEmbaracosa {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while(scanner.hasNext()) { // retorna true se foi informado uma entrada no console
            String string = scanner.nextLine(); // leia a entrada informada

            boolean isSubstring = false;

            for(int i=0; i <= string.length() - 1; i++) {

                if( string.substring(0, i).endsWith(string.substring(i)) ) {
